...
window.slugify = function(str) {
	...
	str = str.replace(/\s+/g, '_') // collapse whitespace and replace by _
...
/* drag and drop */
	...
	getHtml: function(keepHelperAttributes = true) 
	{
		...
		//html += doc.getElementById('main--content').innerHTML;

		html += doc.getElementsByClassName("main--content")[0].innerHTML;
	...
	{
		var data = {};
		console.log("File name: '"+fileName+"' FM: '"+Vvveb.FileManager.getCurrentFileName()+"'");
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(url, null, function (data) {
			$('#message-modal').modal().find(".modal-body").html("Failas buvo išsaugotas: " + data);
		}, saveUrl);		
	},
...
//Pages, file/components tree 
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( e ) {
			...
			data["name"] = $(this).find("[name=file]").val();
			data["title"] = data.name;
			data["url"] = "/" + slugify(data.name);
			data["startTemplateUrl"] = $(this).find("[name=startTemplateUrl]").val();
			data["file"] = "//home/u794216831/domains/mokyklelepasaka.lt/public_html/beta/resources/views/landing/" + slugify(data.name) + ".blade.php";

			console.log(data);

			Vvveb.FileManager.addPages([data]);
			e.preventDefault();
