...
use App\Http\Controllers\BilingualismConsultationController;
use App\Http\Controllers\CouponController;
use App\Http\Controllers\GroupQuestFormController;
use App\Http\Controllers\Pages\AboutLessonsPageController;
use App\Http\Controllers\Pages\BilingualismConsultationPageController;
...
Route::get('/apie-pamokas', function () {
    return view('landing_new.apie_pamokas_naujas')->with('siteContent',
...
Route::get('/suaugusiuju-kursai', function () {
    return view('landing_new.suagusiuju_kursai_naujas')->with('siteContent',
...
Route::get('/susitikimai', function () {
    return view('landing_new.susitikimai_naujas')
        ...
        ->with("before", \App\Models\Introduction::where('date_at', '<', \Carbon\Carbon::now('utc'))->orderBy('date_at',
            'desc')->get())
        ->with("coming", \App\Models\Introduction::where('date_at', '>', \Carbon\Carbon::now('utc'))->orderBy('date_at',
            'asc')->get())
...
Route::post('/select-group/order/free/create/{slug}',
    [OrderController::class, 'createFreeOrder'])->middleware(['auth']);
Route::get('/select-group/order/free/success/{slug}',
    [OrderController::class, 'showSuccessPage'])->middleware(['auth'])->name('orderFreeSuccess');
...
Route::post('/lietuviu-kalbos-pamokos/anketa',
    [GroupQuestFormController::class, 'submitResults'])->name('lithuanian-language-form-submit');
Route::post('/lietuviu-kalbos-pamokos/perkrauti',
    [GroupQuestFormController::class, 'reset'])->name('lithuanian-language-form-reset');


Route::group(['prefix' => 'dashboard', 'middleware' => ['auth']], static function () {
    Route::get('', function () {
        return view('dashboard.index')->with("meetings",
            \App\Models\Meeting::orderBy('date_at', 'asc')->where("date_at", ">",
                \Carbon\Carbon::now('utc')->subMinutes(120))->orderBy("date_at", "ASC")->get());
    })->name('home');
    Route::get('', function () {
        return view('dashboard.index')->with("meetings",
            \App\Models\Meeting::orderBy('date_at', 'asc')->where("date_at", ">",
    ...
    //consultations
    Route::get('/groups/consultations/{slug}',
        [BilingualismConsultationController::class, 'show'])->name('groups-consultation');
    Route::post('/groups/consultations/upload/{id}/delete', [
        BilingualismConsultationController::class, 'deleteFile',
    ])->name('consultation-delete-homework-file')->middleware('auth');
    Route::post('/groups/consultations/upload', [
        BilingualismConsultationController::class, 'uploadFile',
    ])->name('consultation-homework-store')->middleware('auth');
    Route::post('/groups/consultations/homework/{id}/edit',
        [BilingualismConsultationController::class, 'editGroupHomework'])->name('consultation-homework-edit');
    ...
    Route::options('/groups/createMessage',
        [GroupController::class, 'createMessage'])->name('create-message-conversations');
    ...
    Route::group(['prefix' => 'pages', 'as' => 'pages.'], static function () {
        Route::get('/', function () {
            return view('dashboard.cms-pages.pages');
        })->name('index');
        ...
        Route::group(['prefix' => 'lithuanian-courses-children', 'as' => 'lithuanian-courses-children.'],
            static function () {
        ...
        Route::group(['prefix' => 'bilingualism-consultation', 'as' => 'bilingualism-consultation.'],
            static function () {
                Route::get('/edit', [BilingualismConsultationPageController::class, 'edit'])->name('edit');
                Route::put('/update', [BilingualismConsultationPageController::class, 'update'])->name('update');
            });
    });
});
