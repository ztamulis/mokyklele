...
<div class="learning--group--select--wrapper w-100" data-vvveb-disabled>
    ...
    @foreach($groups as $key => $group)
        <div class="learning--group--select--row" data-group="{{ $group->type }}">
            ...
            <div class="text">
                <a style="color: #000000;!important;" @if($group->students()->count() >= $group->slots) href="javascript:;" @else onclick="chooseLessonDataLayer(
                        '{{$group->name}}',
                {{$group->id}},
                {{$group->adjustedPrice()}},
                        '{{$group->type}}',
                ...
                @if($group->time_2)
                    / <a @if($group->students()->count() >= $group->slots)  @else onclick="chooseLessonDataLayer(
                            '{{$group->name}}',
                    {{$group->id}},
                    {{$group->adjustedPrice()}},
                            '{{$group->type}}',
