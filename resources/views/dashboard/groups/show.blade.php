            ...
            <div class="client--dashboard--title--text">
                <h3>{{$group->name}}
                    <small>
                        ...
                        @endphp
                        @if (!empty($nextLesson))
                            {{ App\TimeZoneUtils::updateTime(\App\Http\Controllers\GroupController::nextLesson($group)->date_at->timezone(Cookie::get("user_timezone", "GMT"))->format("Y-m-d H:i"), $nextLesson->updated_at) }} <small>({{ Cookie::get("user_timezone", "GMT") }})</small>
                        @endif
                    </small>
                </h3>
                        ...
                        @foreach($group->events()->where("date_at", ">" ,\Carbon\Carbon::now('utc')->addHours(3))->orderBy("date_at","ASC")->get() as $event)
                            <div class="dashboard--time">
                                ...
                                <div class="dashboard--time--info">
                                    ...
                                    {{ \App\TimeZoneUtils::updateTime($eventDate->format("Y-m-d H:i"), $event->updated_at) }}
                                </div>
                            </div>
                        @endforeach
                    </div>
