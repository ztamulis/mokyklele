...
@section("title", "Nemokama pamoka")

@section("content")
    <div class="landing--col--even-1">
        <img src="/images/landing/castle.webp" alt="Castle">
    </div>
    <div class="landing--col--even-2">
        <h1>Išbandykite<br>nemokamai</h1>
        <p>
            Pabandykite nemokamai – vaikas sudalyvaus smagioje lietuvių kalbos pamokoje, o mokytojas įvertins vaiko kalbos mokėjimo lygį. Vėliau galėsime pasiūlyti tinkamiausią grupę tolimesniam mokymuisi <a href="/pavasario-kursas">pavasario kurse</a>!
        </p>
        <p>
            <a href="/pavasario-kursas">Jau bandėte nemokamą pamoką?<br>Registruokitės kursui čia </a>
        </p>
    </div>
    <div class="clear"></div>
    <div class="learning--group--select--wrapper">
        ...
        @if ($errors->any())
            <script>alert("Klaida! @foreach ($errors->all() as $error) {{ $error }} @endforeach");</script>
        @endif
        <form method="post" action="/register-free" class="free--lesson" data-vvveb-disabled>
            @csrf
            <div class="landing--col--even-1">
                <input type="text" name="name" placeholder="Jūsų vardas" required value="{{ old("name") }}">
                <br>
                <input type="text" name="student_name" placeholder="Vaiko vardas" required value="{{ old("student_name") }}">
                <br>
                Kiek vaikui metų?
                <br>
                <label><input type="radio" name="student_age" value="2-4"> 2-4 metai</label><br>
            ...
            <div class="landing--col--even-2">
                ...
                <select name="country" required>
                    ...
                    <option @if(\Illuminate\Support\Str::startsWith($country, "United")) selected @endif value="United Kingdom">United Kingdom</option>
                    <option @if(\Illuminate\Support\Str::startsWith($country, "Ukraine")) selected @endif value="Ukraine">Ukraine</option>
                    <option @if(\Illuminate\Support\Str::startsWith($country, "United")) selected @endif value="United Arab Erimates">United Arab Emirates</option>
                    <option @if(\Illuminate\Support\Str::startsWith($country, "United")) selected @endif value="United States of America">United States of America</option>
                ...
                <br>
                <textarea placeholder="Papildomi komentarai" name="comments">{{ old("comments") }}</textarea>
            </div>
            <div class="clear"></div>
            <br><br>
            <label><input type="checkbox" name="terms" value="1" @if(old("terms")) checked @endif > Sutinku su Pasakos privatumo politika</label><br>
    ...
    <div class="landing--col--even-1">
        ...
        <h2>
            Mano vaikas nesupranta ir nekalba lietuviškai.
            Ar turite jam grupę?
        </h2>
        <p>
        ...
        <p>
            Pamokos vyksta kartą per savaitę, dažniausiai savaitgaliais. Pamokos trukmė – 30-40min. Pamokų laikas visada nurodomas Didžiosios Britanijos laiku (GMT+1).
        </p>
        <p>
    ...
    <div class="landing--col--even-2">
        ...
        <p>
            Jeigu broliai/sesės yra toje pačioje Pasaka klasėje (grupės skirstomos pagal amžių), mokėti reikės tik už vieną vaiką.
        </p>
        <h2>
        ...
        <p>
            <a href="/apie-pamokas">Daugiau apie mūsų pamokas →</a>
        </p>
    </div>
    <div class="clear"></div>
@endsection